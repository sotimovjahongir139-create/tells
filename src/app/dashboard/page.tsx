...
async function getData() {
  const [stats, lastSync] = await Promise.all([
    prisma.amoCallDailyStat.findMany({
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-4 md:p-8 space-y-6 max-w-5xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold">Qo'ng'iroqlar Holati</h1>
          {s && (
            <p className="text-sm text-gray-400 mt-1">
              Sana: {fmtDate(s.statDate)} · {s.managerName}
            </p>
          )}
        </div>
      ...
      {!s ? (
        <div className="flex items-center justify-center h-64 text-gray-500">
          Ma'lumot mavjud emas. Sinxronizatsiya tugmasini bosing.
        </div>
      ) : (
        <>
